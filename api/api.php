...
if ($_POST['type'] == 'EditRating')
{
    ...
    try {
        $checkStmt = $conn->prepare("SELECT * FROM ratings WHERE product_id = ? AND user_id = ?");
    ...
    try {
        ...
        $stmt = $conn->prepare("UPDATE ratings SET rating = ?, comment = ? WHERE product_id = ? AND user_id = ?");
